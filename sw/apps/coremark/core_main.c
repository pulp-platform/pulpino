...
__attribute__ ((section(".heapsram"))) ee_u8 static_memblk[TOTAL_DATA_SIZE*MULTITHREAD];
#endif
...
__attribute__ ((section(".heapsram"))) core_results results[MULTITHREAD];
...
MAIN_RETURN_TYPE main(int argc, char *argv[]) {
  ...
  if(get_core_id() == 0) {
    ...
    for (i=0 ; i<MULTITHREAD; i++) {
      results[i].memblock[0]=static_memblk+i*TOTAL_DATA_SIZE;
