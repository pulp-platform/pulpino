...
def brev_encode(input, reg_src, reg_dst, length, radix, iteration, inst_str):
    ...
    # Simple check
    # if reg_dst == reg_src:
    #     inst_str += "\t"+14*" "+"\"mv %%[c], x%s \\n\" : [c] \"=r\" (res) : : \"x%s\");" % (reg_dst, reg_dst)
    # else:
    #     inst_str += "\t"+14*" "+"\"mv %%[c], x%s \\n\" : [c] \"=r\" (res) : : \"x%s\", \"x%s\");" % (reg_dst, reg_src, reg_dst)
    # inst_str += "\n\n\tcheck_uint32(result, \"brev\", res,  res_brev[%s]);\n\n" % (str(iteration))
    # Store/Load check
    if reg_dst == reg_src:
        inst_str += "\t"+14*" "+"\"sw x%s, 0(%%[addr])\\n\"\n" % (reg_dst)
        inst_str += "\t"+14*" "+": : [addr] \"r\" (&res_vec[%s]) : \"x%s\");\n" % (iteration, reg_src)
    else:
        inst_str += "\t"+14*" "+"\"sw x%s, 0(%%[addr])\\n\"\n" % (reg_dst)
        inst_str += "\t"+14*" "+": : [addr] \"r\" (&res_vec[%s]) : \"x%s\", \"x%s\");\n" % (iteration, reg_src, reg_dst)
        
    return inst_str

def write_test_in_file_old(inst_str):
...
def write_test_in_file(inst_str):
    ...
    # inst_str.append("")
    # inst_str += "}\n\n"
    #  Find the generated lines and keep everything up to that line
    for line in file:
        if(re.findall("// GENERATED BY gen_stimuli.py", line)):
            file_lines.append(line)
            found_gen = True
            break
        file_lines.append(line)
    if(found_gen):
        file_lines.append(inst_str)
        file_lines.append("\n")
        # Delete previously generated lines and replace them with new ones
        for line in file:
            if(re.findall("// STOP GENERATED BY gen_stimuli.py", line)):
                file_lines.append(line)
                break
        # Keep lines after generated part
        for line in file:
            file_lines.append(line)
    else:
        # No generated part was found
        # Try old replacement function
        write_test_in_file_old(inst_str)
        return
    file.close()
    file = open(file_loc, "w")
    file.writelines(file_lines)
...
for i in range(0,NumberOfStimuli):
    ...
    rnd_src = random.randint(5,10)
    rnd_dst = random.randint(5,10)
