...
/*
#define SATURATION (1 << DATA_WIDTH)-1
*/
...
void  __attribute__ ((noinline)) Conv3x3_Vector  (Pixel * In_Img, Pixel * Out_Img, int R, int C, Filtc  * Kernel)
{
  ...
  coeff_0 = (FiltcV)  {Kernel[0], Kernel[1]};
  coeff_1 = (FiltcV)  {Kernel[3], Kernel[4]};
  coeff_2 = (FiltcV)  {Kernel[6], Kernel[7]};
  ...
  for (c=0; c < C-2; c++) {
    r = 0;
    Img_0   = (PixelV)  {      In_Img[c], In_Img[c+1]     };
    Img_1   = (PixelV)  {    In_Img[c+R], In_Img[c+1+R]   };
    Img_2   = (PixelV)  {  In_Img[c+2*R], In_Img[c+1+2*R] };
    ...
    for (r=1; r < R-2; r++) {
        ...
        S = S >> FILT_SHIFT; //divide by 2^FILT_SHIFT
        //saturation, S in (0, 2^(17-1) -1) -> (0,0xFFFF)
        S = clipu(S,0,SATURATION);
...
void  __attribute__ ((noinline)) Conv5x5_Vector  (Pixel * In_Img, Pixel * Out_Img, int R, int C, Filtc  * Kernel)
{
  ...
  coeff_0  = (FiltcV)  {  Kernel[0],  Kernel[1]};
  coeff_1  = (FiltcV)  {  Kernel[2],  Kernel[3]};
  coeff_2  = (FiltcV)  {  Kernel[5],  Kernel[6]};
  coeff_3  = (FiltcV)  {  Kernel[7],  Kernel[8]};
  coeff_4  = (FiltcV)  { Kernel[10], Kernel[11]};
  coeff_5  = (FiltcV)  { Kernel[12], Kernel[13]};
  coeff_6  = (FiltcV)  { Kernel[15], Kernel[16]};
  coeff_7  = (FiltcV)  { Kernel[17], Kernel[18]};
  coeff_8  = (FiltcV)  { Kernel[20], Kernel[21]};
  coeff_9  = (FiltcV)  { Kernel[22], Kernel[23]};
  ...
  for (c=0; c < C-4; c++) {
    ...
    Img_0   = (PixelV)  {             In_Img[c],     In_Img[c+1]      };
    Img_1   = (PixelV)  {           In_Img[c+2],     In_Img[c+3]      };
    Img_2   = (PixelV)  {           In_Img[c+R],     In_Img[c+1+R]    };
    Img_3   = (PixelV)  {         In_Img[c+R+2],     In_Img[c+3+R]    };
    Img_4   = (PixelV)  {         In_Img[c+2*R],     In_Img[c+1+2*R]  };
    Img_5   = (PixelV)  {       In_Img[c+2*R+2],     In_Img[c+3+2*R]  };
    Img_6   = (PixelV)  {         In_Img[c+3*R],     In_Img[c+1+3*R]  };
    Img_7   = (PixelV)  {       In_Img[c+3*R+2],     In_Img[c+3+3*R]  };
    Img_8   = (PixelV)  {         In_Img[c+4*R],     In_Img[c+1+4*R]  };
    Img_9   = (PixelV)  {       In_Img[c+4*R+2],     In_Img[c+3+4*R]  };
